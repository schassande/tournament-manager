  ...
  template: `
    <div>
      <p-selectbutton
        ...
        [(ngModel)]="selectedDay"
        optionLabel="label"
        (ngModelChange)="selectedDayChanged()"
      />
        ...
        @for (part of parts(); track part.part.id) {
          ...
          >
            ...
            <ng-template #body let-game let-ri="rowIndex">
              <tr
                class="tableRowItem  tableRowItem-{{ game.timeslot!.id % 2 }}"
              >
                <td
                  [pEditableColumn]="game.timeSlot"
                  pEditableColumnField="timeslot"
                  ...
                  <p-cellEditor>
                    <ng-template #input>
                      <p-select
                        id="timeslot"
                        size="small"
                        [options]="part.playingTimeSlots"
                        [(ngModel)]="game.timeSlot"
                        optionLabel="name"
                        optionValue="id"
                        appendTo="body"
                        placeholder="time slot"
                        (onChange)="timeSlotSelected(part, game, $event.value)"
                      >
                ...
                <td
                  [pEditableColumn]="game.field"
                  pEditableColumnField="field"
                  style="text-align: center;"
                >
                  <p-cellEditor>
                    <ng-template #input>
                      <p-select
                        ...
                        appendTo="body"
                        placeholder="Field"
                        (onChange)="fieldSelected(part, game, $event.value)"
                      >
                ...
                <td
                  [pEditableColumn]="game.division"
                  pEditableColumnField="division"
                  style="text-align: center;"
                  *ngIf="showDivision()"
                >
                  <p-cellEditor>
                    <ng-template #input>
                      <p-select
                        id="division"
                        size="small"
                        [options]="tournament()!.divisions"
                        [(ngModel)]="game.division"
                        optionLabel="name"
                        optionValue="id"
                        appendTo="body"
                        placeholder="Division"
                        (onChange)="divisionSelected(game, $event.value)"
                      >
                    ...
                    <ng-template #output>{{
                      game.division.shortName
                    }}</ng-template>
                  </p-cellEditor>
                </td>

                <td
                  [pEditableColumn]="game.game.what"
                  pEditableColumnField="what"
                  style="text-align: center;"
                >
                  <p-cellEditor>
                    <ng-template #input
                      ><input
                        pInputText
                        type="text"
                        [(ngModel)]="game.game.what"
                        (blur)="gameWhatChanged(game)"
                    /></ng-template>
                ...
                <td
                  [pEditableColumn]="game.homeTeam"
                  pEditableColumnField="homeTeam"
                  style="text-align: center;"
                >
                  <p-cellEditor>
                    <ng-template #input>
                      <p-select
                        id="homeTeam"
                        size="small"
                        [options]="game.division!.teams"
                        [(ngModel)]="game.homeTeam"
                        optionLabel="name"
                        optionValue="id"
                        appendTo="body"
                        placeholder="Team"
                        (onChange)="teamSelected(game, true, $event.value)"
                      >
                ...
                <td
                  [pEditableColumn]="game.awayTeam"
                  pEditableColumnField="awayTeam"
                  style="text-align: center;"
                >
                  <p-cellEditor>
                    <ng-template #input>
                      <p-select
                        ...
                        size="small"
                        [options]="game.division!.teams"
                        [(ngModel)]="game.awayTeam"
                        optionLabel="name"
                        optionValue="id"
                        appendTo="body"
                        placeholder="Team"
                        (onChange)="teamSelected(game, false, $event.value)"
                      >
                        <ng-template let-team #item #selectedItem>
                          <div class="flex items-center gap-2">
                            <div>{{ team.shortName }}</div>
                ...
                <td style="text-align: center;">
                  <i
                    class="pi pi-trash action action-remove"
          ...
          <div class="buttonPanel">
            <p-button
              ...
              icon="pi pi-add"
              label="Add a game"
              [disabled]="deletingPartDayId() !== null"
            ></p-button>
            @if (deletePartDayId() === part.part.id) {
              <p-button
                label="Confirm delete all games"
                icon="pi pi-check"
                severity="danger"
                (click)="deleteAllGames(part)"
                [loading]="deletingPartDayId() === part.part.id"
              />
              <p-button
                label="Cancel"
                severity="secondary"
                (click)="cancelDeleteAllGames()"
                [disabled]="deletingPartDayId() !== null"
              />
            } @else {
              <p-button
                ...
                icon="pi pi-trash"
                severity="danger"
                (click)="requestDeleteAllGames(part)"
                [disabled]="deletingPartDayId() !== null || !part.games.length"
              />
            }
          </div>
        }
...
export class TournamentGamesComponent extends AbstractTournamentPage {
  ...
  deletePartDayId = signal<string | null>(null);
  deletingPartDayId = signal<string | null>(null);
  ...
  constructor() {
    ...
    effect(() => {
      if (this.tournament()) {
        ...
        this.days.update(() =>
          this.tournament()!.days.map((day: Day) => {
  ...
  /** Opens the confirmation state for deleting one PartDay's games. */
  requestDeleteAllGames(part: PartView): void {
    if (this.deletingPartDayId() !== null || !part.games.length) return;
    this.deletePartDayId.set(part.part.id);
  }

  /** Cancels the pending deletion confirmation. */
  cancelDeleteAllGames(): void {
    if (this.deletingPartDayId() === null) this.deletePartDayId.set(null);
  }

  /** Deletes every game belonging to the selected PartDay after confirmation. */
  deleteAllGames(part: PartView): void {
    if (
      this.deletePartDayId() !== part.part.id ||
      !this.selectedDay ||
      this.deletingPartDayId() !== null
    ) {
      return;
    }
    this.deletingPartDayId.set(part.part.id);
    this.gameService
        ...
        next: (games) => {
          ...
          Promise.all(partGames.map((game) => this.gameService.delete(game.id)))
            .then(() => {
              ...
              this.deletingPartDayId.set(null);
              this.loadGames();
            })
            .catch(() => {
              this.deletePartDayId.set(null);
  ...
  private loadGames() {
    ...
    // console.log('loadGames', this.selectedDay);
    this.gameService
      .byDay(this.tournament()!.id, this.selectedDay.value.id)
      .pipe(
        map((games: Game[]) => {
            ...
            this.selectedDay!.value.parts.map((part: PartDay) => {
                ...
                .map((game: Game) => {
                  ...
                  return {
                    ...
                    timeslotStr: timeslot
                      ? this.dateService.toTime(timeslot.start)
                      : '',
                    field: this.tournament()!.fields.find(
                      (field: Field) => field.id === game.fieldId,
                    ),
                    homeTeam,
                    awayTeam,
                  };
              ...
              if (part.allFieldsAvaillable) {
                ...
                part.availableFieldIds.forEach((fieldId: string) => {
                  const f = this.tournament()!.fields.find(
  ...
  addGame(pv: PartView) {
    ...
    if (pv.games.length > 0) {
      ...
      if (lastGV.field) {
        const fieldIndex = pv.fields.findIndex(
          (field: Field) => field.id === lastGV.game.fieldId,
        );
        if (0 <= fieldIndex && fieldIndex < pv.fields.length - 1) {
          ...
          const timeslotIndex = pv.playingTimeSlots.findIndex(
            (ts: Timeslot) => ts.id === lastGV.game.timeslotId,
          );
          if (
            0 <= timeslotIndex &&
            timeslotIndex < pv.playingTimeSlots.length - 1
          ) {
            console.debug(
              'use the next timeslot and the first field',
              timeslotIndex + 1,
            );
  ...
  timeSlotSelected(part: PartView, gv: GameView, timeslotId: string) {
    ...
    gv.timeslot = part.playingTimeSlots.find(
  ...
  divisionSelected(gv: GameView, divisionId: string) {
    ...
    gv.division = this.tournament()!.divisions.find(
      (division: Division) => division.id === divisionId,
    );
