...
5. `RefereeSelector` owns transient widget state such as popover visibility, search text, focused card, and the derived visible list. The page-scoped `RefereeSelectorFacade` owns the active level/category/gender/upgrade/player-referee/eligibility filters and sort order, which are shared by selector instances and reapplied when a popover is opened. The initial search text supplied by keyboard activation remains contextual and is not persisted. On opening, the widget focuses the text input after the popover is rendered.
